...
| **BIP 43** | Purpose Field | ✅ Implemented | Purpose `84'` for native SegWit derivation |
| **BIP 44** | Multi-Account HD | ✅ Superseded by BIP84 | Previously used `m/44'/20'/...`; migrated to BIP84 for native SegWit compatibility |
| **BIP 84** | Native SegWit HD | ✅ Implemented | Path `m/84'/20'/0'/change/index`; coin type 20 per SLIP-44 registration |
...
- **Compatibility**: Seeds generated by this wallet can be imported into any BIP39/BIP84 compatible wallet using coin type 20

### Digi-ID Derivation

Digi-ID uses a separate derivation path per site (not BIP 84):
